...
public class Parser {
    ...
    private boolean expr(){
       ...
       // System.out.print("Err in : " + position);

        if(init() || assign() || cust_for() || cust_while() || cust_if() || init_list() || functions() ||print()) {
            expr = true;
        }
    ...
    private boolean init_list(){
        ...
        if(getNextLexeme() == Lexeme.INIT_LIST){
            tableOfCollections.put(tokens.get(position).getValue(), new List());
